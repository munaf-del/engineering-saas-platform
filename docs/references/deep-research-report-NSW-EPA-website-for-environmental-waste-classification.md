...
```mermaid
flowchart TD
  A[Confirm activity triggers licensing] --> B[Gather required application inputs]
  B --> C[Submit application via online licensing portal]
  C --> D[Regulator assesses application]
  D --> E{Further information needed?}
  E -- Yes --> F[Provide additional information]
  F --> D
  E -- No --> G{Decision}
  G -- Granted --> H[Licence issued with conditions]
  H --> I[Ongoing obligations: monitoring, reporting, annual returns]
  G -- Refused/withdrawn --> J[End / re-scope project]
```
