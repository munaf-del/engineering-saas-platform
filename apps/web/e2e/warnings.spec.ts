...
test.describe('Warning and Review States', () => {
  ...
  test('project detail shows missing rule packs warning', async ({ page }) => {
    ...
    const hasWarning = await page
      .getByText('Missing Approved Rule Packs')
      .isVisible()
      .catch(() => false);
    const noWarning = !(await page
      .getByText('Missing Approved Rule Packs')
      .isVisible()
      .catch(() => false));
  ...
  test('calculation detail shows engineer review banner for failures', async ({ page }) => {
    ...
    const hasReviewBanner = await page
      .getByText('Engineer Review Required')
      .isVisible()
      .catch(() => false);
    const noReviewBanner = !(await page
      .getByText('Engineer Review Required')
      .isVisible()
      .catch(() => false));
