...
test.describe('Snapshot Pinning', () => {
  ...
  test('steel sections page shows snapshot status card', async ({ page }) => {
    ...
    const hasData = await page
      .getByText('Catalogue Snapshot Status')
      .isVisible()
      .catch(() => false);
    if (hasData) {
  ...
  test('steel sections page shows pinned badge for active catalogue', async ({ page }) => {
    ...
    const hasActive = await page
      ...
      .isVisible()
      .catch(() => false);
    const hasNoCatalogs = await page
      .getByText('No catalogues')
      .isVisible()
      .catch(() => false);
...
test.describe('Project Standards Pinning', () => {
  ...
  test('project standards page shows pinned dates', async ({ page }) => {
    ...
    const isEmpty = await page
      .getByText('No standards assigned')
      .isVisible()
      .catch(() => false);
  ...
  test('shows rule pack status for each assigned standard', async ({ page }) => {
    ...
    const hasAssignments = await page
      .getByText('Rule pack loaded')
      .first()
      .isVisible()
      .catch(() => false);
    const hasMissing = await page
      .getByText('No rule pack')
      .first()
      .isVisible()
      .catch(() => false);
    const isEmpty = await page
