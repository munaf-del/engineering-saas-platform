...
export default function ProjectSpatialSheetsPage({ params }: { params: Promise<{ id: string }> }) {
  const { id: projectId } = use(params);
  const searchParams = useSearchParams();
  const { data: project, isLoading, error } = useProject(projectId);
  const returnToHref = normalizeSpatialSheetsReturnTo(searchParams.get('returnTo'));
...
function normalizeSpatialSheetsReturnTo(value: string | null) {
