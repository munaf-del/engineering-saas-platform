...
import { ArrowLeft, Save, Weight } from 'lucide-react';
...
export default function LoadCasesPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setIsDirty(false);
      toast.success('Project load cases saved');
    } catch {
      toast.error('Failed to save project load cases');
    }
  }
  ...
  return (
    <>
      ...
      <PageHeader
        title="Load Cases"
        description={`${project.code ?? ''} · Shared project load actions used across calculators`}
        badges={
          <>
