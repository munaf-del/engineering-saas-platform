...
describe('drafting command session', () => {
  ...
  it('starts a secant pile wall baseline command waiting for the first vertex', () => {
    expect(startDraftingSecantPileWallCommand()).toEqual({
      phase: 'waiting_first_point',
      points: [],
      previewPoint: null,
  ...
  it('captures the first secant pile wall baseline vertex before committing', () => {
  ...
  it('updates secant pile wall baseline preview from the next pointer vertex', () => {
    const firstPoint = commitDraftingPathCommandPoint(
      startDraftingSecantPileWallCommand(),
      'secant_pile_wall',
      { x: 0, y: 0 },
    ...
    expect(getDraftingCommandTool(preview)).toBe('secant_pile_wall');
    expect(getDraftingCommandPreviewPoints(preview)).toEqual([
      { x: 0, y: 0 },
      { x: 1400, y: 250 },
    ]);
  ...
  it('commits a secant pile wall through the manual generated-wall baseline boundary', () => {
    const firstPoint = commitDraftingPathCommandPoint(
      startDraftingSecantPileWallCommand(),
      'secant_pile_wall',
      { x: 0, y: 0 },
    );
    const result = commitDraftingPathCommandPoint(firstPoint.session, 'secant_pile_wall', {
      x: 1800,
      y: 300,
    });
    ...
    if (result.committed && result.tool === 'secant_pile_wall') {
      expect(result.tool).toBe('secant_pile_wall');
      expect(result.placement).toEqual({
        baselinePoints: [
          { x: 0, y: 0 },
          { x: 1800, y: 300 },
      ...
      expect(result.placement).not.toHaveProperty('overlapMm');
  ...
  it('does not finish a secant pile wall until the existing two-point minimum is met', () => {
    const firstPoint = commitDraftingPathCommandPoint(
      startDraftingSecantPileWallCommand(),
      'secant_pile_wall',
      { x: 0, y: 0 },
  ...
  it('ignores duplicate/no-op secant pile wall baseline vertices without crashing', () => {
    const firstPoint = commitDraftingPathCommandPoint(
      startDraftingSecantPileWallCommand(),
      'secant_pile_wall',
      { x: 100, y: 100 },
    );
    const duplicate = commitDraftingPathCommandPoint(firstPoint.session, 'secant_pile_wall', {
      x: 100,
      y: 100,
    });
  ...
  it('cancels an incomplete secant pile wall command without committing', () => {
    const firstPoint = commitDraftingPathCommandPoint(
      startDraftingSecantPileWallCommand(),
      'secant_pile_wall',
      { x: 0, y: 0 },
  ...
  it('switches from an incomplete secant pile wall command to a primitive command without committing', () => {
    const firstPoint = commitDraftingPathCommandPoint(
      startDraftingSecantPileWallCommand(),
      'secant_pile_wall',
      { x: 0, y: 0 },
  ...
  it('preserves secant pile wall baseline snap refs and optional z and rl metadata', () => {
    const start: DraftingPoint = {
      x: 0,
      y: 0,
      z: 11.5,
      rl: 11.5,
      snapRef: {
        sourceObjectId: 'secant-baseline-setout-1',
        anchorKind: 'endpoint',
        anchorIndex: 0,
    ...
    const result = commitDraftingPathCommandPoint(firstPoint.session, 'secant_pile_wall', end);
